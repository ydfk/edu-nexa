...
func Create(c *fiber.Ctx) error {
	...
	if err := ensureGradeNameUnique(database, strings.TrimSpace(req.Name), ""); err != nil {
...
func Update(c *fiber.Ctx) error {
	...
	if err := ensureGradeNameUnique(database, strings.TrimSpace(req.Name), item.Id.String()); err != nil {
		return response.Error(c, err.Error())
	}
...
func Delete(c *fiber.Ctx) error {
	...
	if err := ensureGradeDeletable(database, item); err != nil {
		return response.Error(c, err.Error())
	}
...
func ensureGradeNameUnique(database *gorm.DB, name string, excludeID string) error {
	var item model.Grade
...
func ensureGradeDeletable(database *gorm.DB, item model.Grade) error {
	var count int64
	if err := database.Model(&classgroupModel.Class{}).Where("grade_id = ?", item.Id.String()).Count(&count).Error; err != nil {
	...
	if err := database.Model(&studentModel.Student{}).Where("grade_id = ?", item.Id.String()).Count(&count).Error; err != nil {
